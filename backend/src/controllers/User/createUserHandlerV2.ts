...
const userService: IUserService = new UserService(userRepository, imageService);
...
const createUserHandlerV2: RouteHandler<typeof createUserRouteV2, {}> = async (c: Context) => {
  try {
    ...
    const userDto: CreateUserDTO = { name, oauth_app, connect_info, profile_text, icon_image };
